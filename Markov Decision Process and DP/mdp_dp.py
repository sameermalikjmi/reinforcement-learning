...
def policy_improvement(P, nS, nA, value_from_policy, gamma=0.9):
    ...
    for i in range(nS):
        ...
        for a in range(nA):
            ...
            for probaility, next_step,reward,terminal in (P[i][a]):

                q[a] += probaility*(reward + gamma*value_from_policy[next_step])
...
print(policy)
# print(policy_iteration(env.P,env.nS,env.nA, random_policy1,1,tol=1e-8))
V = np.zeros(env.nS)
# print(value_iteration(env.P,env.nS,env.nA, V,1,tol=1e-8))
# print(render_single(env,policy,False))
# # print(policy)
